...
def main():
    ...
    gray_folder = os.path.join(args.save_folder + 'cosine_combinate/', 'gray')
    color_folder = os.path.join(args.save_folder + 'cosine_combinate/', 'color')
...
def net_process(model, image, target, mean, std=None):
    ...
    if True:
        ...
        adver_input = model_immer_attack_auto_loss_combination(input, target, model, attack, args.number_of_steps, args.test_gpu[0])
