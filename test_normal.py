...
def main():
    ...
    if not args.has_prediction:
        ...
        model = torch.nn.DataParallel(model, device_ids=[args.test_gpu[0]]).cuda()
...
def net_process(model, image, target, mean, std=None):
    ...
    if std is None:
        ...
        for t, m, s in zip(input, mean, std):
            t.sub_(m).div_(s)
    input = input.unsqueeze(0).to("cuda:" + str(args.test_gpu[0]))
    target = target.unsqueeze(0).to("cuda:" + str(args.test_gpu[0]))
